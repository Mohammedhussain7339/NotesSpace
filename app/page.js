...
function Page() {
  ...
  const [texts, setTexts] = useState('Hi');
...
<div className='bg-background w-full h-screen flex justify-center items-center'>
  <div className='w-[95%] sm:w-[85%] lg:w-[70%] h-[90vh] rounded-3xl bg-white overflow-hidden flex shadow-lg'>
    <aside className='w-44 h-full border border-l-2 border-background flex justify-center relative'>
      <FaCirclePlus className='text-[50px] my-12 cursor-pointer' onClick={() => setAnimate(!animate)} />
      <ul className='absolute top-40'>
        <li 
    ...
    <main className='bg-blue-200 w-full h-full px-4 py-4 lg:px-12'>
      <h1 className='text-4xl mx-4 my-3 sm:text-3xl md:text-4xl'>Notes</h1>
      <div className='flex flex-wrap justify-center items-center gap-4'>
        {notes.map((item) => (
          <div
            key={item.id}
            className={`w-[200px] sm:w-[250px] h-[250px] rounded-2xl ${item.color} shadow-md relative flex flex-col items-center justify-center p-4`}
          >
            <textarea
              className={`${item.color} outline-none w-[80%] h-[76vh] sm:w-[90%]`}
